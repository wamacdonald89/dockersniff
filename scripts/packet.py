...
cipher_suites = { 'GROUP'   : b'\x00\x0f\xac\x00',
                  'WEP'     : b'\x00\x0f\xac\x01',
                  'TKIP'    : b'\x00\x0f\xac\x02',
                  'CCMP'    : b'\x00\x0f\xac\x04',
                  'WEP'     : b'\x00\x0f\xac\x05' }

auth_suites = { 'MGT' : b'\x00\x0f\xac\x01',
...
class Beacon(AccessPointPacket):
    ...
    def parse_packet(self):
        ...
        while isinstance(elt_layer, Dot11Elt):
            ...
            elif elt_layer.ID == 221 and elt_layer.info.startswith(b'\x00P\xf2\x01\x01\x00'):
                crypto.add("WPA")
            elt_layer = elt_layer.payload  # Check for more Dot11Elt packets within
        if not crypto:
            if 'privacy' in cap:
